...
<div class="container fullbody">
	<div class="col-md-12">
		...
		<div class="card">
			<div class="card-header">
				<h3>Violence/Incident Support List</h3>
			</div>
			<!-- <div class="card-body">
                <div id="DocumentResults"></div>
                <script id="document-template" type="text/x-handlebars-template">
                <table class="table-sm table-bordered table-striped dt-responsive" style="width: 100%" id="batchdatatable">
                    <thead>
                        <tr>
							@{{{thsource}}}
                        </tr>
...
<script type="text/javascript">
	$(document).ready(function(){
        var dTable = $('#data-table').DataTable({
            ...
            serverSide: true,
            ajax: {
                url:'{{ route("support.getIncidentDatatable") }}',
                data: function (d) {
                    d._token      				= "{{ csrf_token() }}";
...
<script type="text/javascript">
	$(document).on('click','#search',function(){
		...
		$.ajax({
			url: "{{route('support.barck.manage.get-incident')}}",
			type: "get",
			data: {
