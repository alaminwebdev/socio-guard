...
<div class="col-xl-12">
	<div class="breadcrumb-holder">
		<h1 class="main-title float-left">Manage Upazila</h1>
			...
			<div class="card-body">
				<table class="table-sm table-bordered table-striped dt-responsive nowrap" style="width: 100%" id="datatable">
					...
					<tbody>
						...
						<tr class="{{$upazila->id}} tr-row">
							<td>{{$key+1}}</td>
							<td>{{$upazila['division']['name']}}</td>
							<td>{{isset($upazila['district']['name']) ? $upazila['district']['name'] : "NO NAME"}}</td>
							<td>{{$upazila->name}}</td>
							<td>
								<a class="btn btn-sm btn-success" title="Edit" href="{{route('setup.upazila.edit',$upazila->id)}}"><i class="fa fa-edit"></i></a>
								<a class="btn btn-sm btn-danger" title="Delete" href="#" onclick="remove({{ $upazila->id }}, event, $(this))"><i class="fa fa-trash"></i></a>
							</td>
						</tr>
						@endforeach
					</tbody>
	...
	function remove(id, e, item)
	{
		...
		swal({
     	...
     	function (isConfirm) {
        	if (isConfirm)
        	{
        		var url  = "{{ route('setup.upazila.delete', '') }}"+"/"+id;
        		// console.log(url);
				...
				$.get(url, function(response) {
					...
					{
						swal("Ops!", "There are some entries found under this Upazila!", "warning");
					}
		      		else
		      		{
