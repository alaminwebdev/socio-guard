	...
	<div class="col-md-12">
		<div class="card">
			<div class="card-header">
				<h5>Previous Violence Type List
					<a class="btn btn-sm btn-success float-right" href="{{route('setup.previous.violence.category.add')}}"><i class="fa fa-plus-circle"></i> Add Previous Violence Type</a>
				</h5>
			</div>
			<div class="card-body">
				<table class="table-sm table-bordered table-striped dt-responsive nowrap" style="width: 100%" id="datatable">
					...
					<tbody>
						...
						<tr class="{{$category->id}} tr-row">
							...
							<td>{{$category->name}}</td>
							<td>
								<a class="btn btn-sm btn-success" title="Edit" href="{{route('setup.previous.violence.category.edit',$category->id)}}"><i class="fa fa-edit"></i></a>
	...
	function remove(id, e, item)
	{
		...
		swal({
     	...
     	function (isConfirm) {
        	if (isConfirm)
        	{
        		var url  = "{{ route('setup.previous.violence.category.delete', '') }}"+"/"+id;
        		// console.log(url);
