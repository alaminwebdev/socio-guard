	...
	<div class="row">
		<div class="col-md-12 card" style="padding-right: 0px;padding-left: 0px;">
			<div class="card-header">
				<h5>Primary Refferal List
					<a class="btn btn-sm btn-success float-right" href="{{route('refferal.create')}}"><i class="fa fa-plus-circle"></i> Add Primary Refferal</a>
				</h5>
			</div>
			<div class="card-body">
				<table class="table-sm table-bordered table-striped dt-responsive nowrap" style="width: 100%" id="datatable">
					...
					<tbody>
						...
						<tr class="{{$item->id}} tr-row">
							...
							<td>{{$item->name}}</td>
							<td>
								<a class="btn btn-sm btn-success" title="Edit" href="{{route('refferal.edit',$item->id)}}"><i class="fa fa-edit"></i></a>
	...
	function remove(id, e, item)
	{
		...
		swal({
     	...
     	function (isConfirm) {
        	if (isConfirm)
        	{
        		var url  = "{{ route('refferal.show', '') }}"+"/"+id;
        		// console.log(url);
