	...
	<div class="col-md-12">
		<div class="card">
			<div class="card-header">
				<h5>Followup List
					<a class="btn btn-sm btn-success float-right" href="{{route('followups.create')}}"><i class="fa fa-plus-circle"></i> Add Followup</a>
				</h5>
			</div>
			<div class="card-body">
				<table class="table-sm table-bordered table-striped dt-responsive nowrap" style="width: 100%" id="datatable">
					...
					<tbody>
						...
						<tr class="{{$item->id}} tr-row">
							...
							<td>{{$item->title}}</td>
							<td>
								<a class="btn btn-sm btn-success" title="Edit" href="{{route('followups.edit',$item->id)}}"><i class="fa fa-edit"></i></a>
	...
	function remove(id, e, item)
	{
		...
		swal({
     	...
     	function (isConfirm) {
        	if (isConfirm)
        	{
        		var url  = "{{ route('followups.show', '') }}"+"/"+id;
        		// console.log(url);
