	...
	<div class="col-md-12">
		<div class="card">
			<div class="card-header">
				<h5>Training Event List
					<a class="btn btn-sm btn-success float-right" href="{{route('training-event.create')}}"><i class="fa fa-plus-circle"></i> Add Training Event</a>
				</h5>
			</div>
			<div class="card-body">
				<table class="table-sm table-bordered table-striped dt-responsive nowrap" style="width: 100%" id="datatable">
					...
					<tbody>
						@foreach($training_event as $key => $item)
						<tr class="{{$item->id}} tr-row">
							<td>{{$key+1}}</td>
							<td>{{$item->name}}</td>
							<td>
								<a class="btn btn-sm btn-success" title="Edit" href="{{route('training-event.edit',$item->id)}}"><i class="fa fa-edit"></i></a>
	...
	function remove(id, e, item)
	{
		...
		swal({
     	...
     	function (isConfirm) {
        	if (isConfirm)
        	{
        		var url  = "{{ route('training-event.show', '') }}"+"/"+id;
        		// console.log(url);
