...
<meta name="meetings" content="{{ json_encode($activityDataObj->training_activity) }}">
{{-- {{ dd($activityData) }} --}}
<form action="{{route('activity.add_step_3',['step'=>4])}}" method="post">
    {{ csrf_field() }}
...
<div class="card custom-card-style">
    ...
    <div id="meeting_container">
        @if (count($activityData) > 0 && count(@$activityData[0]->training_activity) == 0)
                        ...
                        <div class="row">
                            <div class="col-md-3">
                                ...
                                <select is_event_type="true" name="event_id[]" offset="0" onchange="checkChange(this)" id="evt_id_1" class=" form-control form-control-sm select2">
                                    ...
                                    @foreach ($training_event as $item)
        ...
        @endif
        @foreach($activityData[0]->training_activity as $key => $value)
            @if ($key+1 < 2)
                <div id="meeting_container_1" class="m-bottom card-body">
                    <div  class="m-top row">
                        <div class="p-top p-left bg-color col-md-10">
...
<div class="text-right">
    <a href="{{route('activity.add',['step'=>2, 'selp_activity_ref' => $activityData[0]->selp_activity_ref])}}" class="btn btn-success" >Back</a>
    {{-- <a href="{{route('data.pollisomaj.add',['step'=>10])}}" class="btn  btn-primary" >Save & Next</a> --}}
    {{-- <a href="{{route('activity.add',['step'=>4])}}" class="btn  btn-success" >Save & Next</a> --}}
...
@include('backend.activity.activity_script',['events'=>$training_event])
<script>
    $( function() {
