...
<meta name="meetings" content="{{ json_encode($activityDataObj->pt_show_activity) }}">

<form action="{{route('activity.add_step_6',['step'=>7])}}" method="post">
    {{ csrf_field() }}
...
<div class="card custom-card-style">
    ...
    <div id="meeting_container">
        @if (count($activityData) > 0 && count(@$activityData[0]->pt_show_activity) == 0)
        ...
        @endif
        @foreach($activityData[0]->pt_show_activity as $key => $value)
            @if ($key+1 < 2)
                <div id="meeting_container_1" class="m-bottom card-body">
                    <div  class="m-top row">
                        <div class="p-top p-left bg-color col-md-10">
...
<div class="text-right">
    <a href="{{route('activity.add',['step'=>5, 'selp_activity_ref' => $activityData[0]->selp_activity_ref])}}" class="btn btn-success" >Back</a>
    ...
    {{-- <a href="{{route('activity.add',['step'=>7])}}" class="btn  btn-success" >Save & Next</a> --}}
...
@include('backend.activity.activity_script',['events'=>$pt_show_event])
<script>
    $( function() {
