...
class ReasonReportController extends Controller
{
    public function reasonReport()
    {
        ...
        // dd($platform->toArray());
        return view('backend.report.reason_report_view', $data);
    }

    public function reasonReportPdf(Request $request)
    {
        ...
        if ($request->reason_all == 1) {
            $sources = ViolenceReason::select('id','name')->orderBy('id', "ASC")->get();
        } else {
            $sources = ViolenceReason::select('id','name')->where('id', $request->reason_id)->orderBy('id', "ASC")->get();
        }
        ...
        if ($request->format_download == 1) {
            ...
            $pdf = PDF::loadView('backend.report.pdf.reason_report', $pdata,[],['title' => 'Certificate','format' => 'A4-L','orientation' => 'L']);
