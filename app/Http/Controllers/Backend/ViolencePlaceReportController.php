...
class ViolencePlaceReportController extends Controller
{
    public function violencePlaceReport()
    {
        $data['platform'] = OrganizationName::where('status', 1)->get();
        $data['regions']  = Region::where('status','1')->get();
        $data['violenceplaces'] = SurvivorIncidentPlace::where('status','1')->get();
        // dd($platform->toArray());
        return view('backend.report.violenceplace_report_view', $data);
    }

    public function violencePlaceReportPdf(Request $request)
    {
        ...
        if ($request->place_all == 1) {
            $sources = SurvivorIncidentPlace::select('id','name')->orderBy('id', "ASC")->get();
        } else {
            $sources = SurvivorIncidentPlace::select('id','name')->where('id', $request->place_id)->orderBy('id', "ASC")->get();
        }
        ...
        foreach ($survivor_infos as $survivor_key => $survivor_info) {
            foreach ($sources as $source_key => $source) {
                foreach ($violences as $violence_key => $violence) {
                    ...
                    if ($survivor_info->survivor_incident_place_id == $source->id && $survivor_info->violence_category_id == $violence->id) {
        ...
        if ($request->format_download == 1) {
            // $pdf = PDF::loadView('backend.report.pdf.violenceplace_report', $pdata);
            $pdf = PDF::loadView('backend.report.pdf.violenceplace_report', $pdata,[],['title' => 'Certificate','format' => 'A4-L','orientation' => 'L']);
            $pdf->SetProtection(['copy', 'print'], '', 'pass');
            return $pdf->stream('document.pdf');
        } else {
            $view_link = 'backend.report.excel.violenceplace-report-excel';
            return Excel::download(new MisReportExport($pdata,$view_link), 'violenceplace_report.xlsx');
        }
    }
}
