...
class RelationshipReportController extends Controller
{
    public function relationshipReport()
    {
        $data['platform'] = OrganizationName::where('status', 1)->get();
        $data['regions']  = Region::where('status','1')->get();
        $data['relationships'] = SurvivorRelationship::where('status','1')->get();
        // dd($platform->toArray());
        return view('backend.report.relationship_report_view', $data);
    }

    public function relationshipReportPdf(Request $request)
    {
        ...
        if ($request->relationship_all == 1) {
            $sources = SurvivorRelationship::select('id','name')->orderBy('id', "ASC")->get();
        } else {
            $sources = SurvivorRelationship::select('id','name')->where('id', $request->perpetrator_relationship_id)->orderBy('id', "ASC")->get();
        }
        ...
        foreach ($survivor_infos as $survivor_key => $survivor_info) {
            foreach ($sources as $source_key => $source) {
                foreach ($violences as $violence_key => $violence) {
                    ...
                    if ($survivor_info->perpetrator_relationship_id == $source->id && $survivor_info->violence_category_id == $violence->id) {
        ...
        // return view('backend.report.pdf.relationship_report', $pdata);
        if ($request->format_download == 1) {
            // $pdf = PDF::loadView('backend.report.pdf.relationship_report', $pdata);
            $pdf = PDF::loadView('backend.report.pdf.relationship_report', $pdata,[],['title' => 'Certificate','format' => 'A4-L','orientation' => 'L']);
            $pdf->SetProtection(['copy', 'print'], '', 'pass');
            return $pdf->stream('document.pdf');
        } else {
            $view_link = 'backend.report.excel.relationship-report-excel';
            return Excel::download(new MisReportExport($pdata,$view_link), 'relationship_report.xlsx');
        }
    }
}
