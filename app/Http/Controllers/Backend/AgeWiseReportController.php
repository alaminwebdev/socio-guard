...
class AgeWiseReportController extends Controller
{
    public function ageWiseReport()
    {
        ...
        // dd($platform->toArray());
        return view('backend.report.age_wise_report_view', $data);
    }

    public function ageWiseReportPdf(Request $request)
    {
        ...
        if ($wherein != null) {
            $survivor_infos = SurvivorIncidentInformation::where($where)->whereIn('employee_district_id',$wherein)->whereBetween('created_at', [$from_date.' 00:00:00',$to_date.' 23:59:59'])->limit(5)->get();
        } else {
        ...
        // if ($request->reason_all == 1) {
        // } else {
        //     $sources = Age::select('id','name')->where('id', $request->reason_id)->orderBy('id', "ASC")->get();
        // }
        $sources = Age::select('id','name')->orderBy('id', "ASC")->get();
        ...
        if ($request->format_download == 1) {
            // $pdf = PDF::loadView('backend.report.pdf.age_wise_report', $pdata);
            $pdf = PDF::loadView('backend.report.pdf.age_wise_report', $pdata,[],['title' => 'Certificate','format' => 'A4-L','orientation' => 'L']);
            $pdf->SetProtection(['copy', 'print'], '', 'pass');
            return $pdf->stream('document.pdf');
        } else {
            $view_link = 'backend.report.excel.age-wise-report-excel';
            return Excel::download(new MisReportExport($pdata,$view_link), 'age_wise_report.xlsx');
        }
    }
}
